...
contract BaseHorseRacing is ERC721, Ownable {
    ...
    uint256 public currentRaceId;
    mapping(uint256 => uint256[]) public raceHorseIds;
    mapping(uint256 => address[]) public racePlayers;

    event HorseMinted(address owner, uint256 tokenId);
    event RaceJoined(uint256 raceId, uint256 horseId);
    event RaceFinished(uint256 raceId, uint256 winnerHorse);
    ...
    function joinRace(uint256 horseId) external payable {
        ...
        currentRaceId++;
        raceHorseIds[currentRaceId].push(horseId);
        racePlayers[currentRaceId].push(msg.sender);
    ...
    function finishRace(uint256 raceId) external {
        ...
        uint256[] memory horseList = raceHorseIds[raceId];
        address[] memory playerList = racePlayers[raceId];
        ...
        for (uint i = 0; i < horseList.length; i++) {
            Horse memory h = horses[horseList[i]];
            uint256 score = uint256(h.speed) * 3 + uint256(h.stamina) * 2 + uint256(h.spirit);
            score += uint256(keccak256(abi.encodePacked(block.timestamp, raceId, i))) % 40;
        ...
        uint256 reward = prizePool * 60 / 100;
        prizePool -= reward;

        if (reward > 0) {
            (bool success, ) = payable(playerList[winner]).call{value: reward}("");
            if (success) {
                horses[horseList[winner]].racesWon++;
                playerWins[playerList[winner]]++;
            }
        }

        emit RaceFinished(raceId, horseList[winner]);

        delete raceHorseIds[raceId];
        delete racePlayers[raceId];
    }
